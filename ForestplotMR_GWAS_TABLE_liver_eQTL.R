...
gwas_res <- fread("/mnt/sda/gobemi01/PheWAS/PheWAS_MR_ANGPTL3/results/MRres/gwas_table/ANGPTL3_eQTL/w1000_r2_0.1/MR_results_ANGPTL3_eQTL_cis_multiSNP_pval.5e-06_w1000_r2.0.1.txt")
...
format_pval_dec = function(x) {format(x, nsmall = 3,scientific=FALSE,digits = 3)}
label[1,4] <- 'P-Value'
label[2,4] <- NA
label[3,4] <- format_pval_sci(apob_res$pval)
label[4,4] <- format_pval_sci(hdl_res$pval)
label[5,4] <- paste0("2.20e-85")
label[6,4] <- format_pval_sci(tg_res$pval)
label[7,4] <- NA
...
forestplot::forestplot(
  ...
  xticks = c(seq(from = -0.15, to = 0.25, by = 0.1)),
  clip = c(0),
  graph.pos = 3,
  lwd.ci = 4,
  lwd.xaxis = 4,
  ...
  title="",
  xlab= substitute(paste("Effect size (95% CI) of genetically-predicted ", italic('ANGPTL3') ," liver expression levels on each outcome")))
